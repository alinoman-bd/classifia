...
<div class="search main-info">
	<div class="row mar-0">
		...
		<div class="col-sm-6 col-md-4 col-lg-3">
			<div class="form-group">
				...
				<div class="ctm-select">
					...
					<label for="forest_mach_type"><span class="asterisk text-danger">This field is required</span></label>
					<input type="hidden" id="forest_mach_type" name="forest_mach_type" class="required" />
					<div class="ctm-option-box">
						@foreach(App\ForestMachinaryType::all() as $type)
						<div class="ctm-option" onclick="setThis('forest_mach_type','<?= $type->type_name?>')">{{$type->type_name}}</div>
						@endforeach
					</div>
				</div>
			</div>
		...
		<div class="col-sm-6 col-md-4 col-lg-3">
			<div class="form-group">
				<label for="inputAddress"> Wheel (caterpillar track) width, mm </label>
				<input type="number" name="wheel_width" id="wheel_width" class="form-control" min="1">
			</div>
		</div>
		<div class="col-sm-6 col-md-4 col-lg-3">
			<div class="form-group">
				<label for="inputAddress"> Range, mm </label>
				<input type="number" name="range" id="range" class="form-control" min="1">
			</div>
		</div>
		<div class="col-sm-6 col-md-4 col-lg-3">
			<div class="form-group">
				<label for="inputAddress"> Lifting moment, kNm </label>
				<input type="number" name="lifting_moment" id="lifting_moment" class="form-control" min="1">
			</div>
		</div>
	</div>
</div>
<div class="additional-info extra-info1">
	<div class="row mar-0">
		...
		<div class="col-12 pad-0 extra-box">
			<?php $n=0; ?>
			@foreach(App\ForestMachinaryFeatureEqTitle::all() as $title)
			<div class="row mar-0">
				<div class="col-12">
					<div class="section-tlt">{{$title->title}}</div>
				</div>
				<?php $n++; ?>
				@foreach(App\ForestMachinaryFeatureEqValue::all() as $value)
...
<script>
	function setThis(filed,item){
			...
			if (filed == 'make') {
				$("#make").val(item);
			}else if(filed == 'forest_mach_type'){
				$("#forest_mach_type").val(item);
